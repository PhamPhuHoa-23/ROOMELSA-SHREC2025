...
class Trainer(object):
    ...
    def load_from_checkpoint(self, path):
        ...
        # Xử lý trọng số của model để loại bỏ "module." prefix
        state_dict = checkpoint['state_dict']
        new_state_dict = {}

        for key, value in state_dict.items():
            if key.startswith('module.'):
                # Loại bỏ tiền tố "module."
                new_key = key[7:]  # 7 là độ dài của "module."
            else:
                new_key = key
            new_state_dict[new_key] = value

        # Load state_dict đã được xử lý
        ...
        state_dict = checkpoint['logit_scale']
        new_state_dict = {}
        ...
        for key, value in state_dict.items():
            if key.startswith('module.'):
                ...
                new_key = key
            new_state_dict[new_key] = value
        self.logit_scale.load_state_dict(new_state_dict) #module.logit_scale = checkpoint['logit_scale']
    ...
    def train_one_epoch(self):
        ...
        for data in self.train_loader:
            ...
            if len(idx) > 0:
                # Kiểm tra xem text_feat có hợp lệ không
                logging.info(f"Text feat shape: {text_feat.shape}")
                logging.info(f"Text feat norm: {torch.norm(text_feat, dim=1)}")  # Kiểm tra norm
                logging.info(f"Text feat min/max: {torch.min(text_feat)}, {torch.max(text_feat)}")

            if self.config.training.use_text_contras and len(idx) > 0:
                if self.config.training.use_text_proj:
            ...
            # Thêm image contrastive loss nếu được bật
            if self.config.training.use_img_contras:
                if self.config.training.use_image_proj:
        ...
        if self.rank == 0: 
            logging.info('Train: text_cotras_acc: {0}'\
