...
def make(config, phase, rank, world_size):
    if config.dataset.name == "Four":
        ...
        dataset = CustomDataset(config, phase)
        if phase == "train":
            batch_size = config.dataset.train_batch_size
            sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, drop_last=True)
        else:
...
class CustomDataset(Dataset):
    def __init__(self, config, phase):
        ...
        if self.phase == "train":
            ...
            if hasattr(config.custom_dataset, "pair_list"):
                with open(config.custom_dataset.pair_list, "r") as f:
                    for line in f:
                        pc_path, text_embed_path = line.strip().split()
            ...
            elif hasattr(config.custom_dataset, "pc_dir") and hasattr(config.custom_dataset, "text_embed_dir"):
                import glob, os
                text_embed_files = sorted(glob.glob(os.path.join(config.custom_dataset.text_embed_dir, "*.npy")))
                for text_path in text_embed_files:
                    base_name = os.path.basename(text_path)
                    uuid1, uuid2 = base_name.split(".")[0].split("_")
                    pc_file = os.path.join(config.custom_dataset.pc_dir, uuid1, uuid2, "normalized_model.npy")

                    if os.path.exists(pc_file):
                        self.data_list.append(
                            {
                                "pc_path": pc_file,
                                "text_embed_path": text_path
                            }
                        )

            self.num_points = config.custom_dataset.num_points
            self.y_up = config.custom_dataset.y_up
            self.use_color = config.custom_dataset.use_color
            self.random_z_rotate = config.custom_dataset.random_z_rotate
            self.normalize = config.custom_dataset.normalize
            self.augment = config.custom_dataset.augment
        else:
    ...
    def __getitem__(self, index):
        ...
        text_feat = np.load(item["text_embed_path"])
        ...
        return {
            ...
            "has_text": True,
            "name": os.path.basename(item["pc_path"]),
            "dataset": "Custom",
            "group": "Custom",
            "texts": None
        }
