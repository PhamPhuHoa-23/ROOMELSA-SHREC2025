...
def main(rank, world_size, cli_args, extras):
    ...
    if use_distributed:
        try:
            ...
            print(f"Cannot initialize process group: {e}")
    ...
    if rank == 0:
        ...
        if config.get("use_wandb", True): wandb.init(project=config.project_name, name=config.trial_name, config=OmegaConf.to_object(config))
    ...
    if config.train:
        ...
        model.cuda(rank)
        if use_distributed: model = DDP(model, device_ids=[rank], output_device=rank, find_unused_parameters=False)
